...
public class armaa_targetingDisruptor extends BaseHullMod {
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (interval.intervalElapsed()) {
            ...
            for (ShipAPI nearShip : targets) {
                if (MathUtils.getDistance(nearShip, ship) <= 1000f && !nearShip.isHulk()) {
    ...
    public String getDescriptionParam(int index, HullSize hullSize) {
        if (index == 0) {
            return "" + DAMAGE_MAP.get(HullSize.FRIGATE) + "x /" + DAMAGE_MAP.get(HullSize.DESTROYER) + "x";
        }
        //if (index == 1) return "" + (int) BEAM_DAMAGE_PENALTY;
        if (index == 1) {
            return "" + DAMAGE_MAP.get(HullSize.CRUISER) + "x /" + DAMAGE_MAP.get(HullSize.CAPITAL_SHIP) + "x";
        }
        if (index == 2) {
            return "" + autoAimMalus + "%";
        }
