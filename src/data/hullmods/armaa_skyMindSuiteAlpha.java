...
public class armaa_skyMindSuiteAlpha extends BaseHullMod {
	
	public static final String DATA_PREFIX = "alpha_core_skymind_check_";
	public static final String coreType = "ALPHA CORE";
	...
	private final List<String> squadChatter_alpha =  MechaModPlugin.squadChatter_alpha;
	...
	public void applyEffectsAfterShipCreation(ShipAPI ship, java.lang.String id)
	{
        for (String tmp : BLOCKED_HULLMODS)
        {
			...
			{
   				MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp, "armaa_skyMindAlpha");
			}
        }
	}
	...
	public String getUnapplicableReason(ShipAPI ship) 
	{
        ...
        if (playerFleet != null)
		{
			...
			if(Global.getSector().getPlayerFleet().getCargo().getCommodityQuantity(ITEM) <= 0 && !data.containsKey(coreKey))
				return "Installation requires 1 ALPHA Core";
        }
		...
		{	
			...
			else 
			{
				...
				for(String s : MechaModPlugin.squadChatter) {
					...
					int size = rand.nextInt(squadChatter_alpha.size());
					String chatter = squadChatter_alpha.get(size);;
					...
					if(data.containsKey(coreKey))
					{
						...
						return;
					tooltip.addSectionHeading(pilot.getAICoreId().toUpperCase(),Alignment.MID, 10);
	...
	public void advanceInCampaign(FleetMemberAPI member, float amount)
	{
		...
		if(!data.containsKey(coreKey))
		{
			...
			PersonAPI core = Misc.getAICoreOfficerPlugin(ITEM).createPerson(ITEM, "remnant", Misc.random);
	...
	{
		CombatUtils.applyForce(fighter,fighter.getFacing(),MathUtils.getRandomNumberInRange(1f,100f));
