...
public class armaa_homingLaserScript extends BaseEveryFrameCombatPlugin
{
    ...
    public armaa_homingLaserScript(ShipAPI ship, float gauge) 
	{
		this.ship = ship;
		this.gauge = gauge;
		...
		{			
			...
			if (waveSprite != null)
			{
				MagicRender.objectspace(
							...
							new Vector2f(),
							new Vector2f(),
							new Vector2f((float)Math.random()*proj.getCollisionRadius(),(float)Math.random()*proj.getCollisionRadius()),
				...
				maxDamage-= proj.getDamage().getDamage();
				maxProjs--;
