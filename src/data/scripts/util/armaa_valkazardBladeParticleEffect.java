...
public class armaa_valkazardBladeParticleEffect extends BaseEveryFrameCombatPlugin {

    private final IntervalUtil interval = new IntervalUtil(0.05f, 0.05f);
    private DamagingProjectileAPI proj;
    private ShipAPI ship;

    public armaa_valkazardBladeParticleEffect(DamagingProjectileAPI proj) {
        this.proj = proj;
        this.ship = ship;
    ...
    public void advance(float amount, List<InputEventAPI> events) {
        ...
        if (proj.isFading()) {
            DamagingExplosionSpec spec = new DamagingExplosionSpec(0.5f, 50f, 25f, 100f, 50f, CollisionClass.HITS_SHIPS_AND_ASTEROIDS, CollisionClass.HITS_SHIPS_AND_ASTEROIDS, 5f, 10f, 1f, 25, new Color(100, 100, 0, 205), new Color(250, 150, 0, 155));
     ...
     */
    //beam particles
    private static final Color PARTICLE_COLOR = new Color(255, 100, 100, 200);
        ...
        if (Math.random() < 1f) {
            ...
            for (int i = 0; i < particleCount; i++) {
                ...
                if (Math.random() < 0.25f) {
                    org.magiclib.util.MagicLensFlare.createSharpFlare(Global.getCombatEngine(),
                            proj.getSource(),
                            spawnPoint,
                            2f,
                            50f,
                            (float) (Math.random() * 360f),
                            new Color(180, 40, 20, 150),
