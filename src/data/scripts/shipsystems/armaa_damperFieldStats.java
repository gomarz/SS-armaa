...
public class armaa_damperFieldStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        stats.getBallisticRoFMult().modifyMult(id, 1f - (1f - mult) * effectLevel);
        stats.getEnergyRoFMult().modifyMult(id, 1f - (1f - mult) * effectLevel);
        if (stats.getEntity() instanceof ShipAPI) {
            ShipAPI ship = (ShipAPI) stats.getEntity();
            for (ShipAPI module : ship.getChildModulesCopy()) {
                module.getMutableStats().getHullDamageTakenMult().modifyMult(id, 1f - (1f - mult) * effectLevel);
                module.getMutableStats().getArmorDamageTakenMult().modifyMult(id, 1f - (1f - mult) * effectLevel);
                module.getMutableStats().getEmpDamageTakenMult().modifyMult(id, 1f - (1f - mult) * effectLevel);
            }
        }
        ShipAPI ship = null;
