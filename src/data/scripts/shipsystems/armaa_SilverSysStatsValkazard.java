...
public class armaa_SilverSysStatsValkazard extends BaseShipSystemScript {
    ...
    private final int PARTICLE_SIZE_MIN = 25;
    private final int PARTICLE_SIZE_RANGE = 25;
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if ((state == State.COOLDOWN) || (state == State.IDLE) || (odData.gauge < 0f) || shutdown) {
            ...
            if ((odData.gauge < 0f) && ((state == State.IN) || (state == State.ACTIVE) || (state == State.OUT)) && !ship.getFluxTracker().isOverloaded()) {
                ...
                for(int i = 0; i < 15; i++) {
                    Vector2f vec = MathUtils.getRandomPointInCircle(ship.getLocation(), 200f);
                ...
                Global.getCombatEngine().applyDamage(ship, ship.getLocation(), EXPLOSION_DAMAGE_MAX + explosionMult, DamageType.ENERGY, (EXPLOSION_DAMAGE_MAX + explosionMult), true, false, ship);
        ...
        if (interval.intervalElapsed()) {
            ...
            MathUtils.getRandomPointInCircle(ZERO, randRange);
