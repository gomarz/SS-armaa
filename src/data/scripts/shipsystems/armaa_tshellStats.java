...
public class armaa_tshellStats extends BaseShipSystemScript {
    ...
    public static final Color JITTER_COLOR = new Color(255, 200, 200, 55);
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if(ship != null && ship.isFrigate())
            ship.getMutableStats().getSystemUsesBonus().modifyFlat(id+"armaa_dest", 1f);
        //ship.getSystem().getSpecAPI()
        float jitterLevel = effectLevel;
        float jitterRangeBonus = 0;
        float maxRangeBonus = 10f;
        if (state == State.IN && !runOnce) {
