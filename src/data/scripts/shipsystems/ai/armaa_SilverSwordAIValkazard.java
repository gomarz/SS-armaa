...
import data.scripts.shipsystems.armaa_SilverSysStatsValkazard;
import data.scripts.util.armaa_utils;
import org.lazywizard.lazylib.MathUtils;
import org.lazywizard.lazylib.combat.CombatUtils;
import org.lwjgl.util.vector.Vector2f;

public class armaa_SilverSwordAIValkazard implements ShipSystemAIScript {
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
            ...
            if (DEBUG) {
                if (engine.getPlayerShip() == ship) {
                    ...
                    engine.maintainStatusForPlayerShip(STATUSKEY4, system.getSpecAPI().getIconSpriteName(),
                            "AI", armaa_SilverSysStatsValkazard.isUsable(ship, system) ? "Usable" : "Unusable", !armaa_SilverSysStatsValkazard.isUsable(ship, system));
        ...
        if (!system.isActive() || system.isStateActive()) {
            ...
            float gauge = armaa_SilverSysStatsValkazard.getGauge(ship);
            ...
            if (system.isActive()) {
                trueFluxLevel *= armaa_utils.lerp(1f, armaa_SilverSysStatsValkazard.CAPACITY_MULT, system.getEffectLevel() * overLevel);
                trueHardFluxLevel *= armaa_utils.lerp(1f, armaa_SilverSysStatsValkazard.CAPACITY_MULT, system.getEffectLevel() * overLevel);
            }
            ...
            if (!ship.getFluxTracker().isOverloadedOrVenting() && armaa_SilverSysStatsValkazard.isUsable(ship, system)) {
                if (system.isActive()) {
