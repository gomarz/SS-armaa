...
//I dont remember what this is even for
//I guess this is the script that debuffs all enemies
// while the other AWACS script applies the damage bonus to the actual target
public class armaa_AWACSCeylon implements EveryFrameWeaponEffectPlugin {
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (ship.getParentStation().isFrigate() || ship.isDestroyer()) {
            adjustedRange *= 0.75;
        }
        ...
        if (ship.getParentStation().getSystem() != null && ship.getParentStation().getSystem().isActive()) {
            ...
            for (ShipAPI target : enemyList) {
                if (target.isAlive() && MathUtils.getDistance(target.getLocation(), ship.getLocation()) <= adjustedRange * effectLevel) {

                    if (!ship.getParentStation().getVariant().hasHullMod("fourteenth")) {
                        if (!activated && !Global.getCombatEngine().hasAttachedFloaty(target)) {
