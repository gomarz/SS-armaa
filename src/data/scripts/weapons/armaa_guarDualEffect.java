...
public class armaa_guarDualEffect implements EveryFrameWeaponEffectPlugin {
	...
	// 1 = Plane, 0 = Robot
	private float transformLevel = 1f, originalRArmPos =0f, originalShoulderPos = 0f, originalRShoulderPos = 0f;	
	private boolean isRobot = false, transforming = false, forcedToMech, transformBlock = false;;
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) 
	{
		...
		if((!ship.getFluxTracker().isOverloaded() && !transforming) || !ship.getFluxTracker().isOverloaded() && transformNow && !transforming)
		{
			...
			if(!ship.getFluxTracker().isVenting() && armaa_utils.isKeyDoubleTapped(ship,engine) 
				|| (ship.getFluxTracker().isVenting() && ship.getFluxLevel() != 0 || (ship.getShield().isOn())) && !isRobot && !transforming)
			{
		...
		if (armL != null) 
		{
			...
			armL.getSprite().setCenterY(ogPosLArm.getY()+15*sineC);
		...
		if (pauldronL != null)
		{
			...
			pauldronL.getSprite().setCenterY(ogPosL.getY()-6*sineC);
