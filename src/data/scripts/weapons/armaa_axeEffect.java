...
public class armaa_axeEffect implements EveryFrameWeaponEffectPlugin, OnFireEffectPlugin, OnHitEffectPlugin {
    ...
    private static final Color MUZZLE_FLASH_COLOR = new Color(237, 127, 90, 50);
    private static final Color MUZZLE_FLASH_COLOR_ALT = new Color(255, 155, 0, 100);
    ...
    private static final Color PARTICLE_COLOR = new Color(235, 100, 0, 174);
    private static final Color BLAST_COLOR = new Color(235, 100, 0, 200);
    private static final Color CORE_COLOR = new Color(235, 127, 0);
    private static final int NUM_PARTICLES = 15;
    //@Override

    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ShipAPI ship = projectile.getSource();
        boolean lastHit = projectile.getWeapon().getAmmo() == 0;
        if (target instanceof ShipAPI) {
            ShipAPI enemy = (ShipAPI) target;
            if (!shieldHit && target instanceof ShipAPI && !enemy.isHulk()) {
                ship.setHitpoints(Math.min(ship.getHullLevelAtDeployment() * ship.getMaxHitpoints(), ship.getHitpoints() + Math.max(damageResult.getDamageToHull() / 10f, 20f)));
                ...
                CombatUtils.applyForce(ship, ship.getFacing() - 180f, Math.min(target.getMass() / 4, 50f));
                ((ShipAPI) target).giveCommand(ShipCommand.TURN_LEFT, null, 0);
                CombatUtils.applyForce(target, VectorUtils.getAngle(projectile.getLocation(),target.getLocation()), Math.min(ship.getMass()*2f,100f));
                ((ShipAPI) target).giveCommand(ShipCommand.TURN_LEFT, null, 0);
            }

            if (lastHit) {
                CombatUtils.applyForce(target, target.getFacing() - 180f, Math.min(target.getMass(), 200f));
                CombatUtils.applyForce(ship, ship.getFacing() - 180f, Math.min(target.getMass(), 400f));
            }
