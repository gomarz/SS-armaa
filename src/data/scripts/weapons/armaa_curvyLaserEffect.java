 ...
 */
public class armaa_curvyLaserEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
																					EveryFrameWeaponEffectPlugin {

	public armaa_curvyLaserEffect() {
	}
	
	protected List<armaa_curvyLaserEffect> trails;
    private List<DamagingProjectileAPI> alreadyRegisteredProjectiles = new ArrayList<DamagingProjectileAPI>();

	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;
		
		Iterator<armaa_curvyLaserEffect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (armaa_curvyLaserEffect curr : trails) {
				if (curr.proj != null) {
		...
		for (int i = 0; i < numIter; i++) { 
			for (armaa_curvyLaserEffect trail : trails) {
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		armaa_curvyLaserEffect trail = new armaa_curvyLaserEffect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<armaa_curvyLaserEffect>();		
		}
		trails.add(0, trail);
...
//			offset.y += vel.y * 1f;
	...
	public armaa_curvyLaserEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;
