...
public class armaa_magnaBurstEffect implements EveryFrameWeaponEffectPlugin 
{
        ...
        if (weapon.isFiring()) {
            ...
            if (!hasFired) {
                ...
                if (particle.intervalElapsed()) {
                    ...
                    engine.addHitParticle(origin, vel, MathUtils.getRandomNumberInRange((float)30.0f*size, (float)(charge*size * 70.0f + 30.0f)), MathUtils.getRandomNumberInRange((float)0.5f, (float)(0.5f + charge)), MathUtils.getRandomNumberInRange((float)0.1f, (float)(0.1f + charge / 10.0f)), new Color(charge/10f, charge/2f, charge));
					engine.addSwirlyNebulaParticle(origin, new Vector2f(0.0f, 0.0f),MathUtils.getRandomNumberInRange((float)30.0f*size, (float)(charge*size * 70.0f + 30.0f)), 1.2f, 0.15f, 0.0f, 0.35f*charge, new Color(charge/2f, charge/10f, charge),false);
                    ...
                    for (int i = 0; i < 5; ++i) {
						 engine.addHitParticle(particleLoc, particleVel, MathUtils.getRandomNumberInRange((float)3.0f, (float)(charge * 3.0f + 3.0f)), MathUtils.getRandomNumberInRange((float)0.5f, (float)(0.5f + charge)), MathUtils.getRandomNumberInRange((float)0.75f, (float)(0.75f + charge / 4.0f)), new Color(charge/10f, charge / 1.5f, charge));
                    }
                }
            }
            if (charge == 1.0f) 
			{
				...
				Vector2f vel = weapon.getShip().getVelocity();
                hasFired = true;
				 engine.addHitParticle(origin, vel, MathUtils.getRandomNumberInRange((float)20.0f*size, (float)(charge*size * 70.0f + 30.0f)), MathUtils.getRandomNumberInRange((float)0.5f, (float)(0.5f + charge)), MathUtils.getRandomNumberInRange((float)0.1f, (float)(0.1f + charge / 10.0f)), new Color(charge/2f, charge/10f, charge));
            }
        } else {
            hasFired = false;
		}
