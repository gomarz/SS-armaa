...
import data.scripts.util.armaa_pocketDimensionScript;
...
public class armaa_mknuckleGuidance extends BaseEveryFrameCombatPlugin {
    ...
    private final MissileAPI proj; //The projectile itself
    ...
    IntervalUtil trailInterval = new IntervalUtil(0.1f, 0.1f);
     ...
     */
    public armaa_mknuckleGuidance(@NotNull DamagingProjectileAPI proj, CombatEntityAPI target) {
    ...
    public void advance(float amount, List<InputEventAPI> events) {
        ...
        if (proj == null || proj.isFading() || proj.getHitpoints() <= 0 || !Global.getCombatEngine().isEntityInPlay(proj)) {
            if(proj.getWeapon().getAmmo() < proj.getWeapon().getMaxAmmo() && (proj.isFading() || proj.getHitpoints() <= 0))
                Global.getCombatEngine().addPlugin(new armaa_pocketDimensionScript("armaa_valkazard_geant_mknuckle_return", MathUtils.getRandomPointInCircle(proj.getSource().getLocation(), 500f), proj.getSource(), proj.getWeapon()));
        ...
        if (!interval.intervalElapsed()) {
            ...
            if (proj.getCollisionClass() == CollisionClass.NONE) {
                proj.setCollisionClass(CollisionClass.MISSILE_NO_FF);
            }
        }
        ...
        if (trailInterval.intervalElapsed()) {
            ...
            if (Math.random() < 0.15f) {
                Global.getCombatEngine().spawnEmpArcVisual(proj.getWeapon().getFirePoint(0), proj.getSource(), proj.getLocation(), proj, 10f, new Color(255, 154, 50, 150), Color.black);
            }
        }
        ...
        if (MathUtils.getDistance(proj.getLocation(), proj.getWeapon().getFirePoint(0)) < proj.getCollisionRadius()) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                Global.getCombatEngine().addHitParticle(proj.getLocation(), vel, (float) Math.random() * 5f, 1f, 1f, new Color(150, 150, 150, 150));
            }
            for (int i = 0; i < 6; i++) {
                ...
                Global.getCombatEngine().addSmokeParticle(proj.getWeapon().getFirePoint(0), new Vector2f(x, y), size, 1.5f, 1.5f, new Color(150, 150, 150, 150));
            }
            Global.getCombatEngine().removeEntity(proj);
            Global.getCombatEngine().removePlugin(this);
        }
    ...
    private Vector2f getApproximateInterception(int calculationSteps) {
        ...
        for (int i = 0; i < calculationSteps; i++) {
            ...
            returnPoint.x = proj.getWeapon().getFirePoint(0).x + (target.getVelocity().x * arrivalTime * INTERCEPT_ACCURACY_FACTOR);
            returnPoint.y = proj.getWeapon().getFirePoint(0).y + (target.getVelocity().y * arrivalTime * INTERCEPT_ACCURACY_FACTOR);
        }
