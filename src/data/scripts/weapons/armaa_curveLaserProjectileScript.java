...
public class armaa_curveLaserProjectileScript extends BaseEveryFrameCombatPlugin {
	...
	//Secondary and primary sway both run in parallel, allowing double-sine swaying if desired
	private static final float SWAY_PERIOD_PRIMARY = .3f;
	...
	private static final float INTERCEPT_ACCURACY_FACTOR = 0.5f;
	 ...
	 */
	public armaa_curveLaserProjectileScript(@NotNull DamagingProjectileAPI proj, CombatEntityAPI target, int beamNo,String beamPoint) {
		this.proj = proj;
		this.target = target;
	...
	private void applySwarmOffset(String beampoint) {
		int i = 40; //We don't want to take too much time, even if we get unlucky: only try 40 times
		boolean success = false;
		if(beampoint != "")
		{
			if(Global.getCombatEngine().getCustomData().get(beampoint) instanceof Vector2f)
			{
				Vector2f target = new Vector2f((Vector2f)Global.getCombatEngine().getCustomData().get(beampoint));
				targetPoint = target;
				success = true;
				return;
			}
		...
		while (i > 0 && target != null) {
			...
			if (CollisionUtils.isPointWithinBounds(potPoint, target)) {
				...
				//String targetPointKey = "armaa_beamTarget_"+beamNo
				Global.getCombatEngine().getCustomData().put(beampoint,targetPoint);//returnhere
				success = true;
				break;
			}
		}
