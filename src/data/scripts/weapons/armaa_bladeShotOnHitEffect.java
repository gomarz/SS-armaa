...
public class armaa_bladeShotOnHitEffect implements OnHitEffectPlugin {

    private static final Color COLOR1 = new Color(0, 255, 0, 255);
    private static final Color COLOR2 = new Color(44, 245, 188, 255);
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            float speed = 250f + (float) Math.random() * 150f; // tweak these numbers
            Vector2f vel = new Vector2f(dir.x * speed, dir.y * speed);
            engine.addHitParticle(point, vel, (float)Math.random()*15f, 1f, 1f, new Color(44,245,188,200));
            if(Math.random() > 0.50f)
            engine.spawnEmpArcVisual(point, null, MathUtils.getRandomPointInCircle(point,175f), null, 5f, COLOR2, new Color(200,255,200,255));        
        ...
        if (target instanceof ShipAPI) {
            ...
            if (hitLevel > 0f) {
                if (MagicRender.screenCheck(0.2f, point)) {
                    ...
                    engine.addHitParticle(projectile.getLocation(), new Vector2f(0, 0), 159, 1, 0.4f, new Color(25, 200, 100));
