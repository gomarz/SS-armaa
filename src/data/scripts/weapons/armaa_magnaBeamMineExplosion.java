...
public class armaa_magnaBeamMineExplosion extends NegativeExplosionVisual.NEParams implements ProximityExplosionEffect{
	
	public void onExplosion(DamagingProjectileAPI explosion, DamagingProjectileAPI originalProjectile) {
		NEParams p = armaa_magnaBeamMineExplosion.createStandardRiftParams("armaa_magnabeam_minelayer", 10f);
		//p.hitGlowSizeMult = 0.5f;
		p.thickness = 50f;
		CombatEngineAPI engine = Global.getCombatEngine();
		//float charge = weapon.getChargeLevel();
		float size = 4;		
		engine.addSwirlyNebulaParticle(originalProjectile.getLocation(), new Vector2f(0.0f, 0.0f),MathUtils.getRandomNumberInRange((float)50f, (float)(.5f*size * 70.0f + 30.0f)), 1.2f, 0.15f, 0.5f, 0.5f, Color.blue,true);
		//armaa_magnaBeamMineExplosion.spawnStandardRift(explosion, p);
        //    engine.addSmoothParticle(originalProjectile.getLocation(), new Vector2f(0,0), 200, 2, 0.07f, Color.white);
        //    engine.addHitParticle(originalProjectile.getLocation(), new Vector2f(0,0), 150f, 1.5f, 0.4f, new Color(255,0,0,255));
		   engine.addHitParticle(originalProjectile.getLocation(), new Vector2f(0,0), 150f, 1.5f, 0.06f, Color.red);
           engine.spawnExplosion(originalProjectile.getLocation(), new Vector2f(0,0), Color.white, 100, .2f);
            engine.spawnExplosion(originalProjectile.getLocation(),new Vector2f(0,0), new Color (255,50,200), 87, 0.5f);		
	}
