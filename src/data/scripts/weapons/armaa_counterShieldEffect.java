...
public class armaa_counterShieldEffect implements EveryFrameWeaponEffectPlugin {
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (ship.getShield() != null) {
            ...
            StatusBarData bar = new StatusBarData(ship, gauge / MAX_GAUGE, new Color(r, g, b, 1f), new Color(r, g, b, 1f), 0f, "CS Charge", 1);
            ...
            if (gauge >= MAX_GAUGE || (ship.getFluxLevel() >= 0.65f && ship.getFluxTracker().isVenting())) {
                if (!isFiring && gauge > 0) {
                    ...
                    //Just check if this weapon has _lv2 in the id name
                    //Making a new script just for extra effect this seems excessive
                    if (weapon.getId().contains("lv2")) {
                        if (waveSprite != null) {
                            MagicRender.objectspace(
                                    waveSprite,
                                    ship,
                        ...
                        DamagingExplosionSpec spec = new DamagingExplosionSpec(1f, 500f, 250f, 1000f, 100f, CollisionClass.HITS_SHIPS_AND_ASTEROIDS, CollisionClass.HITS_SHIPS_AND_ASTEROIDS, 10f, 20f, 1f, 100, new Color(0, 100, 250, 205), new Color(0, 150, 250, 155));
                        engine.spawnDamagingExplosion(spec, ship, ship.getLocation());
                        for (int i = 0; i < 10; i++) {
                            ...
                            engine.spawnExplosion(ship.getLocation(), vel, new Color(0, 150, 250, 155), (int) (Math.random() * 360), 2f);
                        }
                    }
                    isFiring = true;
