...
public class armaa_higEffect2 implements EveryFrameWeaponEffectPlugin, OnFireEffectPlugin, OnHitEffectPlugin {
			...
			for(int i = 0; i< numbarrels; i++)
			{
				...
				{				
					...
					{					
						...
						engine.addHitParticle(origin, vel, MathUtils.getRandomNumberInRange((float)20.0f*size, (float)(charge*size * 60.0f + 20.0f)), MathUtils.getRandomNumberInRange((float)0.5f, (float)(0.5f + charge)), MathUtils.getRandomNumberInRange((float)0.1f, (float)(0.1f + charge / 10.0f)), new Color(1f, charge / 4f, charge /1f));
						...
						for (int j = 0; j < 5; ++j) 
						{
							 engine.addHitParticle(particleLoc, particleVel, MathUtils.getRandomNumberInRange((float)1.0f, (float)(charge * 2.0f + 1.0f)), MathUtils.getRandomNumberInRange((float)0.5f, (float)(0.5f + charge)), MathUtils.getRandomNumberInRange((float)0.75f, (float)(0.75f + charge / 4.0f)), new Color(charge, charge/2f, charge/1.2f));
						}
					}
				}
				if (charge == 1.0f) 
    ...
    private static final Color CORE_COLOR = new Color(255, 0, 0);
    private static final Color FLASH_COLOR = new Color(255, 255, 225);
    private static final int NUM_PARTICLES = 10;
	//@Override
	public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine)  
	{
        
       // engine.spawnExplosion(point, new Vector2f(), new Color(255,255,255,150), 100f, 1f);
		...
		{
			engine.spawnExplosion(point, new Vector2f(), PARTICLE_COLOR, 155f, 1f);
			engine.addSmoothParticle(projectile.getLocation(), new Vector2f(0,0), 250, 2, 0.1f, new Color(255,255,255,200));
