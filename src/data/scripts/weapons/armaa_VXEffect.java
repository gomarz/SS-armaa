...
public class armaa_VXEffect implements EveryFrameWeaponEffectPlugin {
    ...
    private WeaponAPI head, armL, armR, pauldronL, pauldronR, dummyGun, realGun, torsoWep, lArmWep, headGlow;
    private final Vector2f ZERO = new Vector2f();

    private float swingLevel = 0f;
    private boolean deathRoll = false;

    private float originalRArmPos = 0f, originalArmPos = 0f, originalShoulderPos = 0f, originalRShoulderPos = 0f;
    ...
    public void init() {
        ...
        for (WeaponAPI w : ship.getAllWeapons()) {
            ...
            if (w.getAnimation() != null && w != dummyGun && w.getId().contains("armaa")) {
                int skin = ship.getVariant().hasHullMod("armaa_vxSkin_AASV") ? 1 : 0;
                if (ship.getVariant().hasHullMod("armaa_vxSkin_midline")) {
                    skin = 2;
                }
                int frames = w.getAnimation().getNumFrames();
                if (frames > 1) {
                    w.getAnimation().setFrame(Math.min(skin, frames - 1)); // never index past the last frame
                }
            }
        }
    }
