 ...
 *
 * Coordinate system: we do NOT set up a custom glOrtho. Instead we use
 * screen pixel dimensions directly so the banner is immune to zoom.
 * LazyFont.draw() is called in the same space.
 *
 ...
 *       new armaa_WarningMessage("WARNING", "ENEMY APPROACHING.  BE ON THE LOOKOUT FOR AN ATTACK.", "ui_wave_incoming"));
 */
public class armaa_WarningMessage extends BaseCombatLayeredRenderingPlugin {
    ...
    public armaa_WarningMessage(String headline, String subtitle, String sound) {
        ...
        for (int i = 0; i < cellDelay.length; i++) cellDelay[i] = rng.nextFloat() * MAX_CELL_DELAY;
    }
    ...
    public void render(CombatEngineLayers layer, ViewportAPI viewport) {
        ...
        for (int c = 0; c < CYCLE_COUNT; c++) {
            ...
            if (pulseT > 0f && pulseT < 1f) drawPulse(vx, vy, vw, vh, pulseT);

            float ba;
            if      (cycleT < 0f)                                  ba = 0f;
            else if (cycleT < CYCLE_BANNER_IN)                     ba = easeOut(cycleT / CYCLE_BANNER_IN);
            else if (cycleT < CYCLE_BANNER_IN + CYCLE_BANNER_HOLD) ba = 1f;
            else if (cycleT < cycleDur)                            ba = 1f - easeOut((cycleT - CYCLE_BANNER_IN - CYCLE_BANNER_HOLD) / CYCLE_BANNER_OUT);
            else                                                    ba = 0f;
    ...
    private void renderBackground(float vx, float vy, float vw, float vh, float alpha) {
        ...
        GL11.glColor4f(0,0,0,vigA); GL11.glVertex2f(vx,    innerBot);
        GL11.glColor4f(0,0,0,vigA); GL11.glVertex2f(vx+vw, innerBot);
        GL11.glColor4f(0,0,0,0);    GL11.glVertex2f(vx+vw, innerBot + vigH);
        GL11.glColor4f(0,0,0,0);    GL11.glVertex2f(vx,    innerBot + vigH);
        GL11.glEnd();
        GL11.glBegin(GL11.GL_QUADS);
        GL11.glColor4f(0,0,0,0);    GL11.glVertex2f(vx,    innerTop - vigH);
        GL11.glColor4f(0,0,0,0);    GL11.glVertex2f(vx+vw, innerTop - vigH);
        GL11.glColor4f(0,0,0,vigA); GL11.glVertex2f(vx+vw, innerTop);
        GL11.glColor4f(0,0,0,vigA); GL11.glVertex2f(vx,    innerTop);
        GL11.glEnd();
        ...
        for (float ly = innerBot; ly < innerTop; ly += 4f) {
            GL11.glVertex2f(vx,    ly);     GL11.glVertex2f(vx+vw, ly);
            GL11.glVertex2f(vx+vw, ly+2f);  GL11.glVertex2f(vx,    ly+2f);
        }
        GL11.glEnd();
    }
    ...
    private void renderText(float vx, float vy, float vw, float vh, float ox, float alpha) {
        ...
        if (headlineW <= 0f) headlineW = fontSize * 5.5f;
        if (headlineH <= 0f) headlineH = fontSize;
    ...
    private void drawPulse(float vx, float vy, float vw, float vh, float pPhase) {
        ...
        GL11.glColor4f(1f, 0.18f, 0f, alpha * 0.85f); GL11.glVertex2f(cx, cy);
        ...
        GL11.glBegin(GL11.GL_TRIANGLE_FAN);
        GL11.glColor4f(0.8f, 0.06f, 0f, alpha * 0.35f); GL11.glVertex2f(cx, cy);
    ...
    private void hline(float x1, float x2, float y) {
        ...
        GL11.glVertex2f(x1, y); GL11.glVertex2f(x2, y);
        GL11.glEnd();
    }
    ...
    private float noiseCellAlpha(float phase, float delay, float globalAlpha) {
        ...
        if (localT < 0.25f && Math.random() < 0.12f) ca *= Math.random();
