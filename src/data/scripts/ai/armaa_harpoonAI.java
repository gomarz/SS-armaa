...
import data.scripts.weapons.armaa_harpoonThrowerEffect;
...
public class armaa_harpoonAI implements MissileAIPlugin, GuidedMissileAI {
    ...
    private final float TEAR_OFF_THRESHOLD = 60f;

    //////////////////////
    //  DATA COLLECTING //
    //////////////////////
    
    public armaa_harpoonAI(MissileAPI missile, ShipAPI launchingShip) {
    ...
    public void advance(float amount) {
        ...
        if(!runOnce){
            runOnce=true;
            List<CombatEntityAPI>list = ((armaa_harpoonThrowerEffect) missile.getWeapon().getEffectPlugin()).getHITS();
            ...
            ((armaa_harpoonThrowerEffect) missile.getWeapon().getEffectPlugin()).setDetonation(anchor);
            ...
            if(anchor==null || ((armaa_harpoonThrowerEffect) missile.getWeapon().getEffectPlugin()).getDetonation(anchor)){
...
//            engine.addFloatingText(missile.getLocation(), ""+(float)(Math.round(dist*10000))/10000,12, Color.green, missile,0.1f,0.1f);
            ...
            if( 
                ...
                missile.setCollisionClass(CollisionClass.MISSILE_FF);
				missile.flameOut();
				Global.getCombatEngine().addFloatingText(missile.getSource().getLocation(), "Harpoon dislodged!", 25f, Color.RED, missile.getSource(), 4f, 1f);
                return;
            }
      ...
      //  }
        ...
        if(missile.getElapsed()>99){            
            missile.setCollisionClass(CollisionClass.MISSILE_FF);
            ((armaa_harpoonThrowerEffect) missile.getWeapon().getEffectPlugin()).applyDetonation(anchor);
            return;
        }
