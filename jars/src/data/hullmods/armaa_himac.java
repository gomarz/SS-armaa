...
public class armaa_himac extends BaseHullMod {
    ...
    static {
		SIZE_MULT.put(HullSize.FIGHTER, 0.8f);
        SIZE_MULT.put(HullSize.FRIGATE, 0.8f);
		SIZE_MULT.put(HullSize.DESTROYER, 1.1f);
...
//        List<ShipAPI> enemyShip = AIUtils.getNearbyEnemies(ship, 500f); // range to scan nearby enemies
        ...
        if (ship.getAI() != null ) {
            ...
            if (data.aiTracker.intervalElapsed() && data.cooldown >= 5f && !ship.getFluxTracker().isOverloaded()) {
				...
				{
					if((flags.hasFlag(AIFlags.MOVEMENT_DEST) && !ship.areAnyEnemiesInRange() || flags.hasFlag(AIFlags.PURSUING))   && !data.assaultBoostEnabled && data.cooldown >= 10f)
					{
						data.assaultBoostCharging = true;
