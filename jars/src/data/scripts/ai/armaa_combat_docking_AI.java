...
public class armaa_combat_docking_AI extends BaseShipAI {
	...
	private void abortOp(ShipAPI ship)
	{
		...
		if(carrier != null && carrier.getHitpoints() <= 0)
    ...
    public void beginlandingAI() 
	{
		...
		if(fighterbay!=null && fighterbay.getWing() != null && fighterbay.getWing().getSource() != null && fighterbay.getWing().getSource().getLandingLocation(fighterbay) != null) 
		{
			landingLocation = fighterbay.getWing().getSource().getLandingLocation(fighterbay); //set to location of carrier in case of drift
		}
		...
		//Works, land in carrier bay by referencing a fighter from that carrier. Unable to grab location data from the carrier it itself.
			...
			{ 
				...
				if(fighterAI.getWing().getSourceShip() == null)
					continue;
				if(fighterAI.getWing().getSourceShip().getHullSpec().hasTag("no_wingcom_docking"))
					continue;
				ShipAPI potentialCarrier = fighterAI.getWing().getSourceShip();
				
				if(potentialCarrier.isFighter() || potentialCarrier.isFrigate() && (!potentialCarrier.isStationModule() || !potentialCarrier.getHullSpec().hasTag("strikecraft_with_bays")) || potentialCarrier == ship || potentialCarrier.getCurrentCR() <= 0 || !potentialCarrier.isAlive())
				{
					continue;
				}
				...
				if (fighterAI != null && potentialCarrier != null) 
				{
					...
					{	
						...
						if(isTaken && fighter != null)
							continue;
						if(potentialCarrier.getLaunchBaysCopy() == null || potentialCarrier.getLaunchBaysCopy().size() <= 0)
							continue;
						bayNo = i;
						if(bayNo > potentialCarrier.getLaunchBaysCopy().size()-1)
							continue;
							
						fighter = potentialCarrier.getLaunchBaysCopy().get(bayNo).getWing().getLeader();
							if(fighter == null || fighter.getWing() == null || fighter.getWing().getSource() == null || fighter.getWing().getSource().getLandingLocation(fighter) == null)
								continue;
						//bayNo = i;					
						...
						if(fighterbay.getWing().getSource().getLandingLocation(fighterbay) != null)
							landingLoc =  fighterbay.getWing().getSource().getLandingLocation(fighterbay);
						else
							landingLoc =  fighterbay.getWing().getSourceShip().getLocation();
