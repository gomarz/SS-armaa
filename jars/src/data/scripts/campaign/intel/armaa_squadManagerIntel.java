...
public class armaa_squadManagerIntel extends BaseIntelPlugin {
	...
	protected void createHelpView(TooltipMakerAPI info) {
		...
		float pad = 3;
		Color h = Misc.getHighlightColor();
		info.addImage("graphics/icons/skills/best_of_the_best.png", 52,opad);		
		info.setParaInsigniaLarge();
		info.addPara("About WINGCOM", opad);
		info.setParaFontDefault();
		info.setBulletedListMode(BaseIntelPlugin.BULLET);
		info.addPara("When an officer is assigned to a unit with WINGCOM, the crew drafted for the squadron can be reviewed here.", pad);
		info.addPara("Squadron Members are generally drafted from the fleets best pilots, often %s than their compatriots.", pad, h, "possessing greater skills");
		info.addPara("By engaging in many battles, these squad members can eventually be %s.", pad, h,"promoted to full fledged officers");
		info.setBulletedListMode(BaseIntelPlugin.BULLET);
		UIPanelAPI temp =info.addImageWithText(pad);
		//info.addImage("graphics/icons/skills/crew_training.png", 52,opad);		
		info.setParaInsigniaLarge();
		info.addPara("Squad Enhancement", opad);
		info.setParaFontDefault();
		info.setBulletedListMode(BaseIntelPlugin.BULLET);
		info.addPara("Squadrons combat performance will fluctuate based on two simple factors.", pad);
		info.addPara("Winning battles will generally increase the squadrons performance by a random amount.", pad, h);
		info.addPara("If the squadron leader is shot down or forced to retreat, performance will suffer.", pad);
		unindent(info);
		info.addImage("graphics/icons/skills/leadership.png", 52,opad);		
		info.setParaInsigniaLarge();
		info.addPara("Promoting Squad Members", opad);
		info.setParaFontDefault();
		info.setBulletedListMode(BaseIntelPlugin.BULLET);
		info.addPara("Squad members can be promoted once their relationship exceeds %s points.", pad,h,"70");
		info.addPara("If a squad member is promoted, a new pilot will fill in their vacant position.",pad);
		info.addPara("You can also simply %s for promotion.",pad,h,"pass them over");
		unindent(info);
