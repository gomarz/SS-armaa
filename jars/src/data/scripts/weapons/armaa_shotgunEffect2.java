        ...
        Vector2f.add((Vector2f)dir.scale(pushYou),target.getVelocity(),target.getVelocity());
			...
			if(MagicRender.screenCheck(0.2f, point)){
				if((float) Math.random() >= 0.90f)
				{
					...
					engine.addHitParticle(projectile.getLocation(), new Vector2f(0,0), 100, 1, 0.4f, new Color(200,100,200));
					engine.spawnExplosion(projectile.getLocation(), new Vector2f(0,0), Color.BLUE, 125, 1);
