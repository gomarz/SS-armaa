...
public class armaa_paEffect implements EveryFrameWeaponEffectPlugin {
	...
	private IntervalUtil boardingInterval = new IntervalUtil(3f,3f);
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) 
	{
		...
		if(boardingInterval.intervalElapsed() && !isBoarding)
		{
			...
			{
				//if(!engine.hasAttachedFloaty(ship))
					//engine.addFloatingTextAlways(ship.getLocation(), "Tango, watch your step. We're about to step onto their turf. Remember, it's shoot first, ask questions later.", 15f, Color.white, ship, 0f, 0f, 10f, 0.5f, 1f, 0f); 
				ship.setShipAI(null);
				...
				engine.getCustomData().put("armaa_boardingTarget_"+boardingTarget.getId()+"_numBoarders",10+numBoarders);
				isBoarding = true;
			}
		}
		...
		if(isBoarding)
		{
			...
			if(!boardingTarget.isAlive() && abscondInterval.intervalElapsed() || boardingTarget == null)
			{
				customData.put("armaa_boardingTarget_"+boardingTarget.getId()+"_numBoarders",-10);
				...
				Global.getSoundPlayer().playSound("fighter_takeoff", 1f, 0.5f, ship.getLocation(),new Vector2f());
				ship.resetDefaultAI();
				isBoarding = false;
			}
			...
			if(sabotageInterval.intervalElapsed())
			{
				...
				{
					engine.addFloatingText(boardingTarget.getLocation(), "Boarded! Defender Strength: " + boardingTarget.getFleetMember().getMinCrew() + " vs " + numBoarders, 15f, Color.white, boardingTarget, 1f, 5f);						
				}		
				engine.applyDamage(boardingTarget,ship.getLocation(),150f,DamageType.ENERGY,150f,true,false,ship);
			}
