...
public class ARMAAWorldGen implements SectorGeneratorPlugin {
    ...
    public void generate(SectorAPI sector) {
        ...
        mek.setRelationship(Factions.PERSEAN, 0.00f);		//
		mek.setRelationship(Factions.INDEPENDENT, -0.1f);	//
        mek.setRelationship(Factions.PIRATES, 0.3f);	// connections with some elements of the pirate groups
		mek.setRelationship(Factions.TRITACHYON, -1f);	// planetkillers
		mek.setRelationship(Factions.REMNANTS, -1f);	// tritachyon equipment
		mek.setRelationship(Factions.DIKTAT, -0.8f);		// despots
		mek.setRelationship(Factions.HEGEMONY, -0.4f);		// funding from some officials?
		mek.setRelationship("tahlan_scalartech", 0.05f);
		mek.setRelationship("tahlan_legioinfernalis", 0.3f);		
    }
}
